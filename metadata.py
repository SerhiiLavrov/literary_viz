...
def get_summarizer():
    ...
    if summarizer is None:
        summarizer = hf_pipeline("text2text-generation", model="google/flan-t5-base")
    return summarizer
...
def search_open_library(title: str, author: str) -> dict:
    try:
        ...
        headers = {"User-Agent": "LiteraryViz/1.0 (student project; contact@example.com)"}
        ...
        if not title_words_match(clean_title, result["title"]):
            print(f"Title mismatch: '{result['title']}' doesn't match '{clean_title}'")
            return {}
...
def extract_title_author_from_lines(raw_lines: list) -> tuple:
    ...
    for idx, line in enumerate(cleaned_lines):
        ...
        if not title and len(line) < 60:
            ...
            capital_ratio = sum(1 for w in capital_words if w[0].isupper()) / max(1, len(capital_words))
...
def extract_metadata(raw_lines: list, full_text: str) -> dict:
    ...
    if ol.get("title"):
        title = ol["title"]
        author = ol.get("author", author) or author
